...
export default async function DashboardPage() {
  const [games, sessions, coachingProfile] = await Promise.all([
    listGames(),
    listSessions(),
    loadCoachingProfile(),
  ]);
  const profileComplete = isProfileComplete(coachingProfile);
  ...
  return (
    <div className="page-shell flex flex-col gap-10 py-8">
      {!profileComplete ? (
        <Link
          href="/settings"
          className="flex flex-wrap items-center justify-between gap-3 rounded-panel border border-line bg-raised px-4 py-3.5 text-sm text-fg-soft transition-colors hover:border-fg-faint"
        >
          <span>
            <span className="font-medium text-fg">Finish your coaching profile</span> — a one-time set
            of questions that shapes how NextRep judges decisions.
          </span>
          <span className="label-caps text-fg-faint">Set up →</span>
        </Link>
      ) : null}
            ...
            <GameRow
              ...
              actions={
                game.origin === "demo" ? (
                  ...
                  <ButtonLink href={`/games/${game.id}/analysis`} variant="secondary" size="sm">
                    Analyze
                  </ButtonLink>
                ) : (
                  <ButtonLink href={`/games/${game.id}/processing`} variant="secondary" size="sm">
