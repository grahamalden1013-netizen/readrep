...
///
/// CryptoKit would do this on iOS, but the commitment scheme is a *game rule*,
/// and game rules live in this package so they can be unit tested anywhere,
/// including on Linux CI where CryptoKit does not exist. It is used only to
/// commit to a two-value secret, never to protect user data.
public enum SHA256 {
    ...
    public static func hash(_ message: [UInt8]) -> [UInt8] {
        ...
        for chunkStart in stride(from: 0, to: padded.count, by: 64) {
            for i in 0..<16 {
                ...
                w[i] = (UInt32(padded[j]) << 24) |
                       (UInt32(padded[j + 1]) << 16) |
                       (UInt32(padded[j + 2]) << 8) |
                       UInt32(padded[j + 3])
            }
            for i in 16..<64 {
